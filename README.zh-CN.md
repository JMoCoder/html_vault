...
默认 Docker 模式面向本地、局域网和私有自托管使用。默认本地/测试账号为 `admin` / `test-password`，并使用开发 session secret。浏览器会先进入登录页，登录后使用 HttpOnly session Cookie。当前不开放注册；生产部署必须替换默认用户名、密码和 session secret。
