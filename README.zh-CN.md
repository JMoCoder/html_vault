...
默认 Docker 模式面向本地、局域网和私有自托管使用。设置 `HTML_VAULT_AUTH_USERNAME`、`HTML_VAULT_AUTH_PASSWORD` 和 `HTML_VAULT_SESSION_SECRET` 后，浏览器会先进入登录页，登录后使用 HttpOnly session Cookie。当前不开放注册；测试用户由部署者在后台配置。
