...
データ設定グループには、ブラウザー側のバックアップ/復元、WebDAV 設定プレースホルダー、JSON エクスポートがあります。静的モードではローカル UI 設定、お気に入り/アーカイブの上書き状態、サイドバー表示設定、現在の Manifest をエクスポートできますが、ディスク上の元 HTML/YAML ファイルはバックアップしません。
