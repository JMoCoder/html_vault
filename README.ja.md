...
既定の認証情報のまま compose スタックを公開インターネットに出さないでください。公開デプロイでは `HTML_VAULT_AUTH_USERNAME`、`HTML_VAULT_AUTH_PASSWORD`、`HTML_VAULT_SESSION_SECRET` を変更し、HTTPS の背後に置いてください。env のユーザー名/パスワードは `data/users.json` が存在しない場合に最初の管理者を作成するためだけに使われ、その後は `users.json` が正になります。Caddy Basic Auth 例として `compose.prod.yml`、`.env.secure.example`、`deploy/caddy-basic-auth.Caddyfile` を用意しています。
