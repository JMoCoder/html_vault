...
既定の Docker モードはローカル、LAN、プライベートセルフホスト向けです。`HTML_VAULT_AUTH_USERNAME`、`HTML_VAULT_AUTH_PASSWORD`、`HTML_VAULT_SESSION_SECRET` を設定すると、ブラウザーは最初にログイン画面を表示し、ログイン後は HttpOnly session Cookie を使用します。登録は無効で、テストユーザーはデプロイ担当者が設定します。
