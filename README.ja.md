...
- ナレッジベース Q&A beta。コンテキスト検索、Markdown 回答表示、ソース表示、会話永続化、現在コンテキストの最新会話復元、コンテキスト別履歴を備えます。
- AI 回答の strict / content expansion モード。strict は選択中のノートブックコンテキストだけで回答し、expansion は外部検索アダプター設定時に明示的な外部ソースを扱います。
- AI 実行履歴、軽量非同期ジョブキュー、キューポップオーバー、Settings 内のグローバル会話管理。
...
- キーワード検索を実装済み。vector/hybrid 検索モードはインターフェイスを用意し、vector store 未設定時はキーワード検索へ自動フォールバックします。
...
- vector store / embedding 検索はスキャフォールド済みで、将来の vector backend が設定されるまではキーワード検索へフォールバックします。
