...
`0.5.0` は最初の安定したセルフホストノートブックリリースです。Docker デプロイ、HTML インポート、メタデータ永続化、フィルター、読書、アーカイブ、公開デプロイの安全境界を中心にしています。
...
既定の compose スタックをそのまま公開インターネットに出さないでください。公開デプロイでは、好みのリバースプロキシで HTTPS と認証を追加してください。Caddy Basic Auth 例として `compose.prod.yml`、`.env.secure.example`、`deploy/caddy-basic-auth.Caddyfile` を用意しています。
...
既定の Docker モードはローカル、LAN、プライベートセルフホスト向けです。既定では `HTML_VAULT_API_TOKEN` を要求しないため、フロントエンドは同一 origin API を直接呼び出せます。

HTML Vault を公開インターネットに出す場合:

- HTTPS の背後に置く。
- アプリを配信する前にログイン、session、その他の認証を要求する。
- バックエンド API 保護用に `HTML_VAULT_API_TOKEN` を設定する。
- リバースプロキシがサーバー側で backend token を注入する。
